...
public class OrgChart {
	...
	private Map<Manager, Set<Employee>> managerHierarchy = new HashMap<>();
    	...
    	if(employee!=null) {
	        if(listOfEmployees.contains(employee)) {
	        	if(employee.hasManager()) {
	            	...
	            	managerHierarchy.get(manager).add(employee);
	        	}
	        	return false;
	        }
	        if(employee.hasManager() && listOfEmployees.contains(employee.getManager())) {
	        ...
	        if(employee.hasManager() && !listOfEmployees.contains(employee.getManager())) {
	        	Manager manager = employee.getManager();
	        	listOfEmployees.add(employee.getManager());
	        	listOfEmployees.add(employee);
	        	if(!managerHierarchy.containsKey(manager)) {
	        		Set<Employee> temp = new HashSet<>();
	        		temp.add(employee);
	        		managerHierarchy.put(manager, temp);
	        	}
	        	managerHierarchy.get(manager).add(employee);
	        	return true;
	        }
	        if(!employee.hasManager() && (employee instanceof Manager)) {
	        	listOfEmployees.add(employee);
	        	if(!managerHierarchy.containsKey(employee))
	        		managerHierarchy.put((Manager) employee, new HashSet<>());
	        	return true;
	        }
	        System.out.println("HEREE"+employee.getName());
    ...
    public boolean hasEmployee(Employee employee) {
        return listOfEmployees.contains(employee);
    }
    ...
    public Set<Employee> getAllEmployees() {
        return new HashSet<Employee>(listOfEmployees);
    }
    ...
    public Set<Employee> getDirectSubordinates(Manager manager) {
        if(managerHierarchy.containsKey(manager))
        	return new HashSet<Employee>(managerHierarchy.get(manager));
    ...
    public Map<Manager, Set<Employee>> getFullHierarchy() {
    	Map<Manager, Set<Employee>> temp = new HashMap<>();
    	for(Map.Entry<Manager,Set<Employee>> entry : managerHierarchy.entrySet()) {
    		temp.put(entry.getKey(), new HashSet<Employee>(entry.getValue()));
